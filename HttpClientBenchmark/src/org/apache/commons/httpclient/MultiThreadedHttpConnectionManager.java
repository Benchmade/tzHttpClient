...
public class MultiThreadedHttpConnectionManager implements HttpConnectionManager {
	...
	// A mapping from Reference to ConnectionSource.  Used to reclaim resources when connectionsare lost to the garbage collector.
	private static final Map REFERENCE_TO_CONNECTION_SOURCE = new HashMap();

	/** The reference queue used to track when HttpConnections are lost to the garbage collector */
	...
	/**
	 * Shuts down and cleans up resources used by all instances of MultiThreadedHttpConnectionManager. All static resources are released, all threads are 
	 * stopped, and {@link #shutdown()} is called on all live instances of MultiThreadedHttpConnectionManager.
	 * @see #shutdown() */
	public static void shutdownAll() {
		...
		synchronized (REFERENCE_TO_CONNECTION_SOURCE) {
			...
			synchronized (ALL_CONNECTION_MANAGERS) {
				// Don't use an iterator here. Iterators on WeakHashMap can get ConcurrentModificationException on garbage collection.
				MultiThreadedHttpConnectionManager[] connManagers = (MultiThreadedHttpConnectionManager[]) ALL_CONNECTION_MANAGERS.keySet().toArray(
						new MultiThreadedHttpConnectionManager[ALL_CONNECTION_MANAGERS.size()]);
				// The map may shrink after size() is called, or some entry may get GCed while the array is built, so expect null.
	 ...
	 * collector.  This method should be called before a connection is released from the connection manager.
	 * 
	...
	private static void shutdownCheckedOutConnections(ConnectionPool connectionPool) {
		...
		// close and release the connections outside of the synchronized block to avoid holding the lock for too long
		for (Iterator i = connectionsToClose.iterator(); i.hasNext();) {
			HttpConnection connection = (HttpConnection) i.next();
			connection.close();
			// remove the reference to the connection manager. this ensures that the we don't accidentally end up here again
	...
	public HttpConnection getConnection(HostConfiguration hostConfiguration) {
		while (true) {
			try {
				...
				// we'll go ahead and log this, but it should never happen. HttpExceptions are only thrown when the timeout occurs and since we have no timeout it should never happen.
	...
	/**
	 * Gets a connection or waits if one is not available.  A connection is available if one exists that is not being used or if fewer than
	 * maxHostConnections have been created in the connectionPool, and fewer than maxTotalConnections have been created in all connectionPools.
	 *
	 * @param hostConfiguration The host configuration specifying the connection details.
	 * @param timeout the number of milliseconds to wait for a connection, 0 to wait indefinitely
	 *
	...
	private HttpConnection doGetConnection(HostConfiguration hostConfiguration, long timeout) throws ConnectionPoolTimeoutException {
		...
		synchronized (connectionPool) {
			...
			// we clone the hostConfiguration so that it cannot be changed once the connection has been retrieved
			...
			while (connection == null) {
				...
				if (hostPool.freeConnections.size() > 0) {
					...
					// have room to add host connection, and there is at least one free connection that can be liberated to make overall room
					...
					connection = connectionPool.createConnection(hostConfiguration);
					// otherwise, we have to wait for one of the above conditions to become true
				} else {
					// TODO: keep track of which hostConfigurations have waiting threads, so they avoid being sacrificed before necessary
					try {
						...
						// Else, do nothing, we were interrupted by the connection pool and should now have a connection waiting for us, continue in the loop and let's get it.
					} finally {
						if (!waitingThread.interruptedByConnectionPool) {
							// Either we timed out, experienced a "spurious wakeup", or were interrupted by an external thread.  Regardless we need to cleanup for ourselves in the wait queue.
	 ...
	 * not exceed the {@link #getMaxConnectionsPerHost() maximum number of connections per host}.
	...
	/**
	 * Gets the total number of pooled connections.  This is the total number of connections that have been created and are still in use by this connection 
	 * manager.  This value will not exceed the {@link #getMaxTotalConnections() maximum number of connections}.
	 * @return the total number of pooled connections
	 */
	 ...
	 * If another thread is blocked in getConnection() that could use this connection, it will be woken up.
	...
	public void releaseConnection(HttpConnection conn) {
		...
		if (conn instanceof HttpConnectionAdapter) {
			...
			// this is okay, when an HttpConnectionAdapter is released is releases the real connection
		}
	...
	private class ConnectionPool {
		...
		/**
		 * Map where keys are {@link HostConfiguration}s and values are {@link HostConnectionPool}s
		 */
		 ...
		 * @param create <code>true</code> to create a pool if not found,<code>false</code> to return <code>null</code>
		 * @return a pool (list) of connections available for the given config, or <code>null</code> if neither found nor created
		 */
		...
		/**
		 * Deletes the given connection.  This will remove all reference to the connection so that it can be GCed.
		 ...
		 * available.  This will wake a thread waiting in this host pool or if there is not one a thread in the connection pool will be notified.
		...
		public void freeConnection(HttpConnection conn) {
			...
			synchronized (this) {
				if (shutdown) {
					// the connection manager has been shutdown, release the connection's resources and get out of here
					conn.close();
					return;
				}
				HostConnectionPool hostPool = getHostPool(connectionConfiguration, true);
	...
	/**
	 * A simple struct-like class to combine the connection list and the count of created connections.
	 */
	...
	private static class WaitingThread {
		...
		/** Flag to indicate if the thread was interrupted by the ConnectionPool. Set to true inside {@link ConnectionPool#notifyWaitingThread(HostConnectionPool)} 
	...
	/**
	 * A thread for listening for HttpConnections reclaimed by the garbage collector.
	 */
	private static class ReferenceQueueThread extends Thread {
		...
		private void handleReference(Reference ref) {
			...
			// only clean up for this reference if it is still associated with a ConnectionSource
			if (source != null) {
				if (LOG.isDebugEnabled()) {
